...
// --- Seeded RNG (JavaScript's Math.random cannot be seeded) ---
...
function gauss(mean, std) { // Gaussian random via Box-Muller
    const u1 = random(), u2 = random();
    return mean + std * Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
}
function shuffle(arr) { // Fisher-Yates shuffle in-place
    for (let i = arr.length - 1; i > 0; i--) {
        const j = Math.floor(random() * (i + 1));
        [arr[i], arr[j]] = [arr[j], arr[i]];
    }
}
function weightedChoice(weights) { // random.choices equivalent
...
const docs = readFileSync('input.txt', 'utf-8').split('\n').filter(l => l.trim());
shuffle(docs);
...
const stateDict = { wte: matrix(vocabSize, nEmbd), wpe: matrix(blockSize, nEmbd), lm_head: matrix(vocabSize, nEmbd) };
...
for (let step = 0; step < numSteps; step++) {
    ...
    process.stdout.write(`\rstep ${String(step + 1).padStart(4)} / ${numSteps} | loss ${loss.data.toFixed(4)}`);
}
