...
function tokenColor(id: number): { bg: string; border: string; text: string } {
  if (id === BOS_ID) {
    return { bg: 'bg-violet-500/20', border: 'border-violet-400/60', text: 'text-violet-300' };
  }
  const t = id / 25; // 0..1
  if (t < 0.33) {
    return { bg: 'bg-cyan-500/20', border: 'border-cyan-400/60', text: 'text-cyan-300' };
  }
  if (t < 0.66) {
    return { bg: 'bg-amber-500/20', border: 'border-amber-400/60', text: 'text-amber-300' };
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <p className="text-slate-300 leading-relaxed text-sm">
          Computers only understand numbers, not letters. A <span className="text-cyan-400 font-medium">tokenizer</span> converts
          text into a sequence of integers called <span className="text-amber-400 font-medium">"token IDs"</span>.
        </p>
        <p className="text-slate-300 leading-relaxed text-sm">
      ...
      <div className="viz-card glow-amber">
        ...
        <div className="grid grid-cols-9 gap-1.5">
          {VOCAB.split('').map((ch, i) => {
            ...
            return (
              ...
              >
                ...
                <span className={isActive ? 'text-amber-400/70' : 'text-slate-600'}> &rarr; </span>
                <span>{i}</span>
              </div>
            );
          })}
          ...
          >
            ...
            <span className={text.length > 0 ? 'text-violet-400/70' : 'text-slate-600'}> &rarr; </span>
            <span>26</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              Real GPT models like ChatGPT use "subword" tokenizers with ~100,000 tokens instead
              of 27. The word <code className="text-emerald-400 bg-slate-800 px-1 py-0.5 rounded text-xs font-mono">"unhappiness"</code> might
              become <code className="text-amber-400 bg-slate-800 px-1 py-0.5 rounded text-xs font-mono">["un", "happiness"]</code> or{' '}
              <code className="text-amber-400 bg-slate-800 px-1 py-0.5 rounded text-xs font-mono">["un", "happ", "iness"]</code>.
              But the principle is identical: <span className="text-cyan-400 font-medium">text &rarr; numbers</span>.
            </p>
          </div>
        </div>
      </div>
